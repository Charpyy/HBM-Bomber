...
public class EntityBomber extends Entity implements IChunkLoader, IConstantRenderer {
	...
	public boolean attackEntityFrom(DamageSource source, float amount) {
		//System.out.println("Source des dégats: " +source);
		if (source.toString().contains("com.paneedah.weaponlib.WeaponSpawnEntity$ProjectileDamageSource")) {
			return false;
		}
	...
	private void killBomber() {
		for(int i = 0; i < 10; i++) {
			ExplosionLarge.spawnParticles(world, this.posX + rand.nextGaussian() * 0.5 - motionX, this.posY + rand.nextGaussian() * 0.5 - motionY, this.posZ + rand.nextGaussian() * 0.5 - motionZ, 3);
		}
		ExplosionLarge.explode(world, posX, posY, posZ, 4, true, false, true);
		System.out.println("Pos X: "+posX + " Pos Y: " + posY + " Pos Z: " + posZ );
	...
	public void spawnCrates(int x, int y, int z, World world ) {
		Class<? extends Entity> entityClass = EntityList.getClass(new ResourceLocation("air_support:parachute_drop"));
		try {
			Entity crate = entityClass.getConstructor(World.class).newInstance(world);
			crate.setPosition(x + rand.nextInt(5), y, z + rand.nextInt(5));
			world.spawnEntity(crate);
			//System.out.println("Dropped parachute at X:" + crate.posX + " Y:" + crate.posY + " Z:" + crate.posZ);
			} catch(Exception e){
	...
	public void onUpdate() {
		...
		if (this.health <= 0) {
			this.setPosition(posX + motionX / 3, posY + motionY / 3, posZ + motionZ / 3);
		}
		else{
			this.setPosition(posX + motionX / 3, posY, posZ + motionZ / 3);
		}
		...
		//	damage = vie - health;
		//	if (getServer() != null) {
		//		String[] PlayerList = getServer().getPlayerList().getOnlinePlayerNames();
		//		for (String playerName : PlayerList) {
		//			EntityPlayerMP player = getServer().getPlayerList().getPlayerByUsername(playerName);
		//			if (player != null) {
		//				player.sendStatusMessage(new TextComponentString("\u00a7c -"+damage+" HP "+health+" Vie total"), true);
		//			}
		//		}
		//	}
		//	vie = health;
		...
		if (getServer() != null) {
			...
			for (String playerName : PlayerList) {
				...
				if (player != null) {
					...
					int y = (int)posY;
					int z = (int)posZ;
					player.sendStatusMessage(new TextComponentString("\u00a78\u00bb \u00a7c Bomber at \u00a7fx:"+x+" y:"+y+" z:"+z), true);
				}
			}
		}

		if(this.health <= 500) {
			ExplosionLarge.spawnParticles(world, this.posX + rand.nextGaussian() * 0.5 - motionX, this.posY + rand.nextGaussian() * 0.5 - motionY, this.posZ + rand.nextGaussian() * 0.5 - motionZ, 1);
		}
		...
		if(this.health <= 0) {
			...
			if (crate) {
				crate = false;
				if (getServer() != null) {
					int x = Double.valueOf(posX).intValue();
					int y = Double.valueOf(posY).intValue();
					int z = Double.valueOf(posZ).intValue();
					for (int i = 0; i < 3; i++) {
						spawnCrates(x, y, z, world);
					}
				}
			}
			...
			if(world.getBlockState(new BlockPos((int)posX, (int)posY, (int)posZ)).isNormalCube() && !world.isRemote) {
				this.setDead();
				if (damageExplo) {
					ExplosionLarge.explodeFire(world, posX, posY, posZ, 10, true, false, true);
				}
				...
				if (getServer() != null) {
					...
					for (String playerName : PlayerList) {
						EntityPlayerMP player = getServer().getPlayerList().getPlayerByUsername(playerName);
						if (player != null) {
							player.sendMessage(new TextComponentString("\u00a78\u00bb \u00a7cLe Bombardier s'est écrasé"));
						}
					}
				}
				crate = true;
		...
		if(this.ticksExisted > timer) {
			...
			if (getServer() != null) {
				...
				for (String playerName : PlayerList) {
					EntityPlayerMP player = getServer().getPlayerList().getPlayerByUsername(playerName);
					if (player != null) {
						player.sendMessage(new TextComponentString("\u00a78\u00bb \u00a7cLe Bombardier s'est échappé"));
					}
				}
			}
		}
	...
	public static EntityBomber openWar(World world, double x, double y, double z, int time, int hp, boolean damage) {
	...
	protected void entityInit() {
		...
		//this.getDataManager().set(STYLE, (byte)6);
		...
		System.out.println("Spawned a Bomber at X:"+posX+" Z:"+posZ+" Y:"+posY+" World:"+world);
		//fac2(world, spawnX, spawnY, spawnZ);
